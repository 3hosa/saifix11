...
class Yemen4GCreditScreen extends StatefulWidget {
  final bool isDarkMode;
  final String title;

  const Yemen4GCreditScreen({
    super.key,
  ...
  @override
  State<Yemen4GCreditScreen> createState() => _Yemen4GCreditScreenState();
}

class _Yemen4GCreditScreenState extends State<Yemen4GCreditScreen> {
          ...
          child: Column(
            ...
            children: [
              ...
              _buildBalanceRow(
                ...
                formatAmountDisplay(double.tryParse(balance) ?? 0.0),
                isBold: true,
              ),
  ...
  Future<void> _processPayment() async {
    ...
    TransactionDetailsBottomSheet.show(
      ...
      currency: 'YER',
      transactionType: 'Yemen 4G Recharge',
      recipientName: recipientName,
      recipientId: _phoneController.text,
      onExecute: () async {
        final auth = await SecurityConfirmationDialog.show(
          context,
          isDarkMode: widget.isDarkMode,
        );
        if (auth != null) _executePayment();
      },
    );
  }
      ...
      if (result['RC'] == 0 || result['RC'] == '0') {
        ...
        if (mounted) {
          await ReceiptDialog.show(
            context,
            isDarkMode: widget.isDarkMode,
            title: 'receipt_yemen_4g_recharge'.tr(),
            ...
            shareText: '✅ إيصال شحن 4G - صيفي باي\nالمرجع: $refNumber',
          );
        }
        ...
        if (mounted) {
          ErrorDialog.show(
            context,
            message: result['MSG'] ?? 'recharge_failed'.tr(),
          );
        }
      }
    } catch (e) {
                    ...
                    child: Form(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 20),
                          _buildSectionTitle('amount_label'.tr()),
                          _buildAmountInput(),
                          _buildAmountWords(),
